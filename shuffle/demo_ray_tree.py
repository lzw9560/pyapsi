...
# 1
# bucket_len = 256
# max_bucket_num = 16 ** 30
# while max_bucket_num:
#     pivot = max_bucket_num - 256
#     # iter = ds.iter_rows()
#     # print("=-=-=")
#     # item = next(iter)
#     # # index = int(int(row["hash_item"][:30], 16)/ npartitions)  
#     # item_hex = int(int(item["hash_item"], 16))
#     # if pivot < item_hex <= max_bucket_num:
#     start = time.time()
#     bucket = []
...
#         if pivot < item_hex <= max_bucket_num:
#             bucket.append(item)
#             # TODO smt encrypt and save 
#             print(len(bucket))
#             encrypt.remote(bucket)
# #             map_outputs = [
# #     encrypt.remote(bucket) for partition in dataset
# # ]
#         else:
#             continue
...
#     max_bucket_num -= 256
#     print(hex(max_bucket_num))
#     print(f"iter bucket execution: {(time.time() - start):.3f}")
